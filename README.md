...
| 3 | `fail2ban.sh` | 安装并配置 Fail2Ban、UFW 与 ipset，对 SSH 爆破进行更强拦截和持久化封禁。 |
...
| 7 | `cloudflare-ssh.sh` | SSH 仅允许 Cloudflare ASN 当前宣告的 IP 前缀访问，同时屏蔽中国 IP 段 ICMP/ICMPv6，并配置定时更新和回滚保护。 |
| 8 | 全部脚本 | 按顺序执行上述功能。 |
